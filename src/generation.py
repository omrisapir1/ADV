...
import re
import math  # added for entropy calculations
...
class AsyncSGLangEngineWrapper:
    ...
    async def _two_phase_for_one_prompt(
        ...
        *,
    ...
    ) -> List[tuple[str, int, float, float]]:  # expanded tuple to include avg_entropy, avg_p_selected
        ...
        phase2_items: List[Tuple[int, str, str, float, float]] = []  # include stats
        for idx, choice in enumerate(resp1.choices):
            ...
            phase2_items.append((idx, think_clean, context, ae, aps))
        if not phase2_items:
            return results
        ...
        for (idx, think_clean, _, ae, aps), resp2 in zip(phase2_items, resp2_list):
            ...
            results[idx] = (full_text, 1, ae, aps)
        return results
    ...
    async def generate_candidates(
        ...
        n_samples: int,
        **gen_cfg: Any,
    ) -> List[List[tuple[str, int, float, float]]]:  # adjusted return type
